...
export const initUserAdditionalInfo = async (
  ...
  try {
    ...
    const message = err.response?.data?.error?.message ?? '사용자 정보 초기화에 실패했습니다.';
...
export const updateUserInfo = async (
  ...
  try {
    ...
    const err = error as AxiosError<{ error: { message: string } }>;
    const message = err.response?.data?.error?.message ?? '사용자 정보 업데이트에 실패했습니다.';
    throw new Error(message);
  }
};
...
export const getUserInfo = async (): Promise<UserResponseDto> => {
  try {
    ...
    const err = error as AxiosError<{ error: { message: string } }>;
    const message = err.response?.data?.error?.message ?? '사용자 정보를 불러오는데 실패했습니다.';
    throw new Error(message);
  }
};
...
export const checkNicknameAvailability = async (nickname: string): Promise<boolean> => {
  try {
    ...
    const err = error as AxiosError<{ error: { message: string } }>;
    const message = err.response?.data?.error?.message ?? '닉네임 확인에 실패했습니다.';
    throw new Error(message);
  }
};
 ...
 * 계정을 삭제합니다.
 */
export const deleteAccount = async (request: UserDeleteRequestDto): Promise<void> => {
  try {
    await api.post('/users/delete', request);
  } catch (error) {
    const err = error as AxiosError<{ error: { message: string } }>;
    const message = err.response?.data?.error?.message ?? '계정 삭제에 실패했습니다.';
    throw new Error(message);
  }
};
...
export const getMyKeywords = async (
  ...
  try {
    ...
    const err = error as AxiosError<{ error: { message: string } }>;
    const message = err.response?.data?.error?.message ?? '키워드 목록을 불러오는데 실패했습니다.';
    throw new Error(message);
  }
}; 